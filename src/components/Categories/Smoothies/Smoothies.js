...
const Smoothies = () => {
    ...
    useEffect(() => {
        const sortArray = (type) => {
            if (type === "priceNewAsc") {
                ...
                const sorted = [...smoothies].sort((a, b) => a[sortProperty] - b[sortProperty])
                setData(sorted);
                ...
                const sortProperty = types[type];
                const sorted = [...smoothies].sort((a, b) => b[sortProperty] - a[sortProperty])
                setData(sorted);
            } else
                if (type === "productNameAsc") {
                    ...
                    const sortProperty = types[type];
                    const sorted = [...smoothies].sort((a, b) => a[sortProperty] === b[sortProperty] ? 0 : a[sortProperty] > b[sortProperty] ? 1 : -1)
                    setData(sorted);
    ...
    const onChange = (value) => {
        setData([...smoothies].filter(item => (
            item.priceNew <= value[1] && value[0] <= item.priceNew
        )))
    };
    const onAfterChange = (value) => {
        setData([...smoothies].filter(item => (item.priceNew <= value[1] && value[0] <= item.priceNew)));
        setValue1(value[0])
        setValue2(value[1])
    };
    const marks = {
    ...
    return (
        <>
            <div className="container">
                <BreadCrumb title="Smoothies" />
            </div>
            <div className="products mb-5">
                <div className="container">
                    <h4 className="text-uppercase">smoothies</h4>
                    <>
