...
const CartOrder = ({ cartProducts, onAdd, onDecrease, onDeleteProduct, totalPrice }) => {
    const onDelete = (product) => {
        onDeleteProduct(product);
    }
    return (
        <div className="cartOrder my-4">
            <h5>Giỏ hàng</h5>
            <table className="table">
                ...
                <tbody>
                    {cartProducts.length === 0 && <tr><td colSpan="5">Vui lòng chọn sản phẩm!!!</td></tr>}
                    {(cartProducts) && (cartProducts).map((item, index) => {
                        return (
                            <tr key={item.id}>
                                <th scope="row">{index + 1}</th>
                                <td ><span>{item.productName}</span><br /> {item.category === "Trà sữa" && <span>Size: {item.size}</span>}</td>
